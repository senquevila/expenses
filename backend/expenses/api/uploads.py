...
class UploadViewSet(viewsets.ModelViewSet):
    queryset = Upload.objects.filter(start_date__gte="2026-01-01")
    ...
    def get_queryset(self):
        ...
        if upload_type:
            qs = qs.filter(upload_type__iexact=upload_type)
        return qs
    ...
    def post_upload_step1(self, request, *args, **kwargs):
        ...
        upload = Upload.objects.create(file=data["file"], identifier=data["identifier"])
    ...
    def get_identifiers(self, request, pk=None, *args, **kwargs):
        """
        This endpoint retrieves all unique identifiers from the Upload instances.
        """
        identifiers = Upload.objects.filter(identifier__isnull=False).values_list("identifier", flat=True).distinct()
        return Response(data={"identifiers": list(identifiers)}, status=status.HTTP_200_OK)
